...
#http://172.16.17.36:54321/
#h2oServer <- h2o.init(ip="172.16.17.36", port = 54321)
...
path <- "/Users/taposhdr/workspace/decision_science/kaggle/ctr_prediction/"
path_submission <- paste0(path,"./data/sampleSubmission")
path_train <- paste0(path,"./data/train")
path_test <- paste0(path,"./data/test")

cat("\nReading data.")
train_ctr_hex <- h2o.importFile(h2oServer, path = path_train, key = "train_ctr.hex")
test_ctr_hex <- h2o.importFile(h2oServer, path = path_test, key = "test_ctr.hex")
...
intcols <- c("C1","banner_pos","site_category","device_type","device_conn_type","C15","C16","C17","C18","C19","C20","C21")
factorcols <- c()

cat("\nAdding features")
train_ctr_hex <- h2o.addNewFeatures(train_ctr_hex, "hour", intcols, factorcols, "train_ctr.hex")
test_ctr_hex <- h2o.addNewFeatures(test_ctr_hex, "hour", intcols, factorcols, "test_ctr.hex")

## Split into train/validation based on training days (first 9 days: train, last day: test)
cat("\nSplitting into train/validation")
train <- h2o.assign(train_ctr_hex[train_ctr_hex$day<30,], 'train')
valid <- h2o.assign(train_ctr_hex[train_ctr_hex$day==30,], 'valid')
h2o.rm(h2oServer, grep(pattern = "Last.value", x = h2o.ls(h2oServer)$Key, value = TRUE))
...
if (usefullmodel) {
  ...
  fullmodel <- h2o.gbm(data=train_ctr_hex, x=c(3:ncol(train)), y=2,
  ...
  pred <- h2o.predict(fullmodel, test_ctr_hex)[,3]
} else {
  cat("\nMaking predictions on test data with model trained on train/validation splits.")
  pred <- h2o.predict(cvmodel, test_ctr_hex)[,3]
}
